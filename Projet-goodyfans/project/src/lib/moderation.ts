// Système de modération automatique avec Azure Content Moderator - Version corrigée pour stack depth
...
// 🎯 NOUVELLE FONCTION: Vérification de la configuration Azure
...
export const moderateImage = async (imageUrl: string): Promise<ModerationResult> => {
  try {
    ...
    // 🎯 SOLUTION: Utiliser l'edge function pour la modération Azure avec timeout
...
// 🎯 FONCTION CORRIGÉE: Modération automatique d'un média spécifique
...
// 🎯 FONCTION COMPLÈTEMENT RÉÉCRITE: Obtenir les statistiques de modération avec requêtes optimisées
export const getModerationStats = async (): Promise<{
  ...
  try {
    ...
    // 🎯 SOLUTION: Utiliser des requêtes COUNT avec select minimal pour éviter le stack depth error
    ...
    // En cas d'erreur, essayer une approche encore plus simple
    try {
      console.log('🔄 Trying fallback stats approach...');
      
      // Essayer juste de compter le total avec une requête très simple
      const { count, error: countError } = await supabase
        .from('content_moderation')
        .select('id', { count: 'exact', head: true });
      
      if (countError) {
        console.error('❌ Even fallback failed:', countError);
      ...
      // Si on peut au moins obtenir le total, retourner ça
      return {
        total: count || 0,
        approved: 0,
        rejected: 0,
      ...
      // Retourner des stats par défaut en dernier recours
      return {
        total: 0,
        approved: 0,
        rejected: 0,
...
// 🎯 FONCTION CORRIGÉE: Modérer tous les médias existants
